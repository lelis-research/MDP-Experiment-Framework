...
class SingleExpAnalyzer:
    ...
    def plot_combined(self, fig=None, axs=None, save_dir=None, show=False, color='blue', marker='-',
        ...
        assert all(c in {"r_e", "r_s", "s_e", "ou_s", "ou_e", 
                         "no_s", "no_e", "uni_ou_e", "uni_ou_s",
                         "cb_hm_first", "cb_hm_last"} for c in plt_configs), \
        ...
        for i, config in enumerate(plt_configs):
            ...
            elif config == "cb_hm_first" or config == "cb_hm_last":
                def pairwise_l2_matrix(E):
                    # E: (N, D)
                    E = np.asarray(E)
                    diff = E[:, None, :] - E[None, :, :]
                    return np.linalg.norm(diff, axis=-1)  # (N, N)
                
                ep_idx = 0 if config == "cb_hm_first" else -1 
                
                cb = [run[ep_idx]["agent_logs"]["code_book"][0] # because there is an extra batch dim in cb
                        for run in self.metrics]
                dists = np.stack([pairwise_l2_matrix(E) for E in cb], axis=0)  # (Runs, N, N)
    ...
    # ---------------------------
    def _plot_heatmap(self, ax, mat, title="", x_label="", y_label="",
                  show_colorbar=True, cbar_label="",
                  vmin=None, vmax=None, aspect="auto",
                  annotate=True, fmt=".2f"):
        """
        Plot a heatmap on a provided axis.

        Extra:
        - Integer ticks on x/y
        - Optional value annotation in each cell
        """
            ...
            for i in range(H):
                for j in range(W):
                    ax.text(
                        ...
                        ha="center",
                        va="center",
                        color="white" if im.norm(mat[i, j]) > 0.5 else "black",
                        fontsize=10
                    )
